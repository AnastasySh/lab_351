...
if ( *p == value)
 и т.д.
 Динамическое выделение памяти — это способ запроса памяти из операционной системы запущенными программами при необходимости. Эта память не выделяется из ограниченной памяти стека программы, а из гораздо большего хранилища, управляемого операционной системой — heap (кучи).
...
int *ptr = new int; // ptr выделяется 4 байта из кучи
int *array = new int[10]; // array выделяется  40 байтов из кучи
Адрес выделяемой памяти передается обратно оператором new и затем он может быть сохранен в указателе.
Последовательные запросы памяти не всегда приводят к выделению последовательных адресов памяти!
int *ptr1 = new int;
int *ptr2 = new int; // ptr1 и ptr2 могут не иметь последовательных адресов
При удалении динамически выделенной переменной, память возвращается обратно в кучу и затем может быть переназначена
(исходя из последующих запросов). Помните, что удаление указателя не удаляет переменную, это просто приводит к возврату памяти 
по этому адресу обратно в операционную систему.

 Для статического массива при описании должно быть определено количество элементов, которое в течение работы программы 
 нельзя изменить (увеличить или уменьшить). При запуске программы, использующей статический массив, на выполнение в оперативной памяти 
 выделяется некоторое количество байт для хранения его элементов. Этот объем памяти будет закреплен за программой до тех пор, пока она не
 закончит свою работу. Даже если эта память не будет использоваться, никакой другой программный код не сможет получить к ней доступ.
Т. Е.1. Размер массива должен быть известен во время компиляции.
2.  Выделение и освобождение памяти происходит автоматически.
3. Часто задаваемые значения массива используют большой объем памяти
Динамический массив позволяет при описании не указывать количество элементов, а определять его во время выполнения программы.
Динамический массив – один из способов рационального распределения ресурсов вычислительной системы. Хотя и обладает некоторыми недостатками.
Во-первых, это не всегда удобная нумерация элементов с нуля. Во-вторых, программисту нужно постоянно понимать в каждой точке программного кода,
в каком состоянии находится динамический массив. Но! Можно создать массив размер которого будет многим больше нежели статический.
